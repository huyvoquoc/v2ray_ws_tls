...
port_alterid_set() {
    if [[ "on" != "$old_config_status" ]]; then
        ...
        [[ -z ${port} ]] && port="443"
        read -rp "Please enter alterID (default: 2 only allows digits) :" alterID
        [[ -z ${alterID} ]] && alterID="2"
    fi
}
modify_path() {
...
web_camouflage() {
    ...
    git clone https://github.com/huyvoquoc/3DCEList.git
...
v2ray_install() {
    ...
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
    wget -N --no-check-certificate https://raw.githubusercontent.com/huyvoquoc/v2ray_ws_tls/${github_branch}/v2ray.sh
...
domain_check() {
    read -rp "Please enter your domain name information (eg: www.itdevops.com):" domain
    ...
    if [[ $(echo "${local_ip}" | tr '.' '+' | bc) -eq $(echo "${domain_ip}" | tr '.' '+' | bc) ]]; then
        ...
        echo -e "${Error} ${RedBG} Please make sure that the correct A record is added to the domain name, otherwise it will not work properly V2ray ${Font}"
...
v2ray_conf_add_tls() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/huyvoquoc/v2ray_ws_tls/${github_branch}/tls/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
v2ray_conf_add_h2() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/huyvoquoc/v2ray_ws_tls/${github_branch}/http2/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
#}
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://raw.githubusercontent.com/huyvoquoc/v2ray_ws_tls/master/ssl_update.sh"
...
basic_information() {
    {
        ...
        echo -e "${Red} Low-level transmission security: ${Font} tls "
    } >"${v2ray_info_file}"
}
show_information() {
    cat "${v2ray_info_file}"
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/huyvoquoc/v2ray_ws_tls/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate https://raw.githubusercontent.com/huyvoquoc/v2ray_ws_tls/${github_branch}/install.sh
...
menu() {
    ...
    2)
        ...
        ;;
    3)
        bash <(curl -L -s https://raw.githubusercontent.com/huyvoquoc/v2ray_ws_tls/${github_branch}/v2ray.sh)
        ;;
    4)
